...
export default function SignInForm() {
  ...
  return (
    ...
    <div className="rounded-2xl bg-white p-5 shadow-xl dark:bg-[#020d24] dark:text-white border border-gray-50 dark:border dark:border-gray-600 w-full max-w-md">
      {/* Mode toggle */}
      <div className="mb-4 grid grid-cols-2 gap-2">
        <button
          type="button"
          onClick={() => setMode("user")}
          className={`cursor-pointer rounded-lg px-3 py-2 border text-sm font-medium transition-colors ${
            mode === "user" 
      ...
      <div className="mb-4 flex gap-2">
        <button
          ...
          className={`cursor-pointer flex-1 rounded-lg px-3 py-2 border text-sm font-medium transition-colors ${
            tab === "signin" 
      ...
      <p className="mb-4 text-sm opacity-80 text-gray-800 dark:text-gray-400">
        {getDescriptionText()}
      </p>
      ...
      {tab === "signin" ? (
        <form onSubmit={onSignin} className="space-y-3">
          <input
            ...
            className="w-full rounded-lg border p-3 bg-gray-50 dark:bg-gray-700 border-gray-700 text-gray-950 dark:text-gray-50 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
            required
          />
          <input
            type="password"
          ...
          <button
            disabled={busy}
            className="w-full rounded-lg bg-bluecustombutton px-4 py-3 font-medium text-white hover:bg-bluecustombuttonhover disabled:opacity-60 transition-colors cursor-pointer"
          >
      ...
      <div className="my-6 flex items-center justify-center gap-3 text-sm opacity-70">
        ...
        <span className="text-gray-700 dark:text-gray-50">atau</span>
        <span className="h-px w-full bg-gray-700" />
      </div>
      ...
      <button
        ...
        disabled={busy}
        className="w-full rounded-lg bg-customgreen dark:bg-greencustombutton dark:text-gray-50 px-4 py-3 font-medium text-gray-50 hover:bg-greencustombuttonhover dark:hover:bg-greencustombuttonhover disabled:opacity-60 transition-colors cursor-pointer"
      >
        Continue with Google
      </button>

      <div className="mt-6 text-center text-sm">
        <a href="/" className="text-gray-400 hover:text-gray-950 dark:hover:text-gray-50 dark:underline">
          Back to Home
        </a>
      </div>
    </div>
